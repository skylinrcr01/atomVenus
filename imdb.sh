#!/bin/bash
# moviedata -- given a movie or tv title, returns a list of mathces. If the user 
# specifies an IMDB numeric index number, returns synopsis of the 
# film instad. USes the IMDB.
...
summarize_film()
{
	# produce an atractive synopsis of the film.
...
######
# Checks wether we're asking for a title by IMDB title number
...
#############
# It's no an IMDB title number, so let's give 'er a search
